<!--- Process uploaded file for decryption --->
	...
	<div id="DecryptFile">
		...
		<div class="content">
		<h3>Decrypt Signed File Processing</h3>
		</div>
		...
		<cfscript>
			...
			stcArgs.privateKeyPath = objH.getKeysDir() & Trim(Form.privateKeyFile);
			stcArgs.publicKeyPath = objH.getKeysDir() & Trim(Form.publicKeyFile);
			stcArgs.targetFile = strUserDir & Trim(Form.ufile) & ".txt"; 
			stcArgs.keyPassPhrase = Trim(Form.pass);
			strUserFileRef = Form.ufile & ".txt";
			
			//run decryption
			strResponse=objPGP.fDecryptOnePassSignature(argumentcollection=stcArgs);
